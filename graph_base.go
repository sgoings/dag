...
// GraphBase is used to provide default behavior for a graph.
...
func (g *GraphBase) Vertices() []Vertex {
...
func (g *GraphBase) EdgesFrom(v Vertex) []Edge {
	var result []Edge
	from := hashcode(v)
...
func (g *GraphBase) EdgesTo(v Vertex) []Edge {
	var result []Edge
	search := hashcode(v)
	for _, e := range g.Edges() {
...
func (g *GraphBase) Remove(v Vertex) Vertex {
...
// is returned.
func (g *GraphBase) Replace(original, replacement Vertex) bool {
...
func (g *GraphBase) RemoveEdge(edge Edge) {
	g.init()
...
func (g *GraphBase) downEdgesNoCopy(v Vertex) Set {
	g.init()
...
// This Set is the same as used internally bu the graph to prevent a copy, and
// must not be modified by the caller.
func (g *GraphBase) upEdgesNoCopy(v Vertex) Set {
	g.init()
...
func (g *GraphBase) Connect(edge Edge) {
	g.init()
...
func (g *GraphBase) StringWithNodeTypes() string {
...
// String outputs some human-friendly output for the graph structure.
func (g *GraphBase) String() string {
	var buf bytes.Buffer
...
func (g *GraphBase) Dot(opts *DotOpts) []byte {
...
type MarshalOpts struct {
	ConnectSubgraphHeads bool
	SubgraphHead         Vertex
	ConnectSubgraphTails bool
	SubgraphTail         Vertex
}

// Marshal returns a json representation of the graph.
func (g *GraphBase) Marshal(opts *MarshalOpts) *marshalGraph {
